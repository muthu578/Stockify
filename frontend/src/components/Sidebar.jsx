...
const Sidebar = () => {
    ...
    return (
        <aside className="w-64 h-screen dark-glass text-white fixed left-0 top-0 flex flex-col z-50 border-r border-white/5">
            <div className="p-8 border-b border-white/5 flex items-center space-x-3">
                <div className="bg-primary-500 p-2.5 rounded-2xl shadow-lg shadow-primary-500/30 animate-pulse-subtle">
                    <ShoppingCart size={24} className="text-white" />
                </div>
                <div>
                    <span className="text-2xl font-black tracking-tighter block leading-none">Stockify</span>
                    <span className="text-[10px] font-black text-primary-400 uppercase tracking-[0.2em]">Enterprise</span>
                </div>
            </div>
                ...
                {filteredMenu.map((item, index) => (
                    <NavLink
                        ...
                        className={({ isActive }) => `
                            ...
                            ${isActive
                                ? 'bg-primary-600/20 text-primary-400 shadow-[inset_0_0_20px_rgba(37,99,235,0.1)] border border-primary-500/20'
                                : 'text-slate-400 hover:bg-white/5 hover:text-white hover:translate-x-1'}
                        `}
                    >
                        {({ isActive }) => (
                            <>
                                <item.icon size={20} className={`${isActive ? 'text-primary-400' : 'text-slate-500 group-hover:text-white'}`} />
            ...
            <div className="p-6">
                <div className="bg-white/5 rounded-3xl p-5 border border-white/5 backdrop-blur-md">
                    <div className="flex items-center space-x-3 mb-4">
                        <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-primary-500 to-primary-700 flex items-center justify-center text-sm font-black shadow-lg">
                            {user?.name?.charAt(0)}
                        </div>
                    ...
                    <button
                        onClick={logout}
                        className="w-full flex items-center justify-center space-x-2 px-4 py-3 text-rose-400 bg-rose-400/10 hover:bg-rose-400 hover:text-white rounded-2xl transition-all duration-300 font-bold text-xs uppercase tracking-widest"
                    >
                        <LogOut size={16} />
                        <span>Logout</span>
                    </button>
